...
$$\text{Moment of a couple} = \text{Force} \times \text{perpendicular distance between the line of action of forces}$$
...
Since, acceleration = change in velocity/time, $a = \frac{(v-u)}{t}$. Hence, we have
...
$$\text{Weight} = \text{Gravitational Force} = \text{mass (m)} \times \text{acceleration due to gravity (g)}.$$
