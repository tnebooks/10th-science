...
#### Activity 10.1
- Take about 50ml of toilet cleaning acid in a beaker
...
*Figure 10.3 Precipitation of PbI₂*

#### Activity 10.2
- Take a pinch of silver nitrate crystals.
- Collect about 5ml of tap water in a test tube.
...
How fast is each reaction? Rank them from the slowest to fastest. How will you determine, which is the fastest and which is the slowest? One of the ways to find out how fast a reaction is as follows: Measure the amount of reactants or products before and after a specific period of time. For example, let us assume that 100g of a substance 'A' undergoes a reaction and after an hour 50g of 'A' is left.

$$\mathrm{A} \rightarrow \mathrm{Product}$$

In another instance, 100g of substance 'C' undergoes a reaction and after an hour, 20g of 'C' is left.

$$\mathrm{C} \rightarrow \mathrm{Product}$$

Can you say which is the faster reaction? In the first reaction, 50g of the reactant (A) is converted into products whereas in the second reaction 80g of the reactant is converted into products in one hour. So, the second reaction is faster. This measurement is called 'the reaction rate'.
...
$$\mathrm{Rate} = -\frac{d[\mathrm{A}]}{dt} = +\frac{d[\mathrm{B}]}{dt}$$
...
2Na(s) + 2HCl(aq) → 2NaCl(aq) + H₂(g)  (fast)

2Na(s) + 2CH₃COOH(aq) → 2CH₃COONa(aq) + H₂(g)  (slow)
...
You will study more about reaction rate in your higher classes.
...
The rate of decomposition of CaCO₃ = The rate of combination of CaO and CO₂

Not only chemical changes, physical changes also may attain equilibrium. When water kept in closed vessel evaporates, it forms water vapour. No water vapour escapes out of the container as the process takes place in a closed vessel. So, it builds up the vapour pressure in the container. At one time, the water vapour condenses back into liquid water and when the rate of this condensation becomes equal to that of vapourisation, the process attains equilibrium.

At this stage, the volume of the liquid and gaseous phases remain constant. Since it is a physical change, the equilibrium attained is called 'Physical Equilibrium'.
...
[H₃O⁺] may be simply written as [H⁺]. Thus the ionic product of water may also be expressed as

$$\mathrm{K_w} = [\mathrm{H^+}][\mathrm{OH^-}]$$
...
*Conceptual Thoughts: Why pure water has pH 7 at room temperature?*
...
$$\mathrm{pH} = -\log_{10}[1 \times 10^{-2}]$$
$$\mathrm{pH} = -(\log_{10}1 - 2\log_{10}10)$$
$$\mathrm{pH} = 0 + 2 \times \log_{10}10$$
$$\mathrm{pH} = 0 + 2 \times 1 = 2$$
$$\mathrm{pH} = 2$$

### pOH

The pOH of an aqueous solution is related to the pH.

The pOH is the negative logarithm of the hydroxyl ion concentration

$$\mathrm{pOH} = -\log_{10}[\mathrm{OH^-}]$$

**Example:** The hydroxyl ion concentration of a solution is 1 × 10⁻⁹ M. What is the pOH of the solution?

**Solution:**
$$\mathrm{pOH} = -\log_{10}[\mathrm{OH^-}]$$
$$\mathrm{pOH} = -\log_{10}[1 \times 10^{-9}]$$
$$\mathrm{pOH} = -(\log_{10}1.0 + \log_{10}10^{-9})$$
$$\mathrm{pOH} = -(0 - 9\log_{10}10)$$
$$\mathrm{pOH} = -(0 - 9)$$
$$\mathrm{pOH} = 9$$
...
| REACTION | TYPE |
|:---------|:-----|
| $$\mathrm{NH_4OH}_{(aq)} + \mathrm{CH_3COOH}_{(aq)} \rightarrow \mathrm{CH_3COONH_4}_{(aq)} + \mathrm{H_2O}_{(l)}$$ | Single Displacement |
| $$\mathrm{Zn}_{(s)} + \mathrm{CuSO_4}_{(aq)} \rightarrow \mathrm{ZnSO_4}_{(aq)} + \mathrm{Cu}_{(s)}$$ | Combustion |
| $$\mathrm{ZnCO_3}_{(s)} \stackrel{\text{Heat}}{\rightarrow} \mathrm{ZnO}_{(s)} + \mathrm{CO}_{2(g)}$$ | Neutralisation |
| $$\mathrm{C_2H_4}_{(g)} + 4\mathrm{O}_{2(g)} \rightarrow 2\mathrm{CO}_{2(g)} + 2\mathrm{H_2O}_{(g)} + \text{Heat}$$ | Thermal decomposition |
...
2. Calculate the pH of 1.0 × 10⁻⁴ molar solution of HNO₃.
